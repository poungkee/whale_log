  ...
  const promptKakaoLogin = useCallback(async () => {
    ...
    try {
      ...
      if (result.type === 'success') {
        ...
        /** 시퀀스 fallback으로 username 부여됐는지 — 카카오 닉네임이 이모지/특수문자뿐일 때만 true */
        const autoGenMatch = url.match(/[?&]usernameAutoGenerated=([01])/);
        ...
        if (tokenMatch && userMatch) {
          ...
          const usernameAutoGenerated = autoGenMatch?.[1] === '1';
          await login(accessToken, user, isNewUser, usernameAutoGenerated);
        } else {
