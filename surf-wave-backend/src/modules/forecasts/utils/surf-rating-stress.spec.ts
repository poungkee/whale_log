...
/**
 * 테스트용 ForecastForRating 생성 헬퍼
 * - swellHeight, swellPeriod, waterTemperature가 누락되면 기본값(null)으로 채움
 * - v1.4에서 추가된 필드 때문에 기존 테스트 데이터와 호환성 유지
 */
function makeForecast(partial: Omit<ForecastForRating, 'swellHeight' | 'swellPeriod' | 'waterTemperature'> & {
...
describe('A. 완벽한 컨디션 (EPIC DAY) — 8점 이상 기대', () => {
  test('A1. 비치 초급 — 파고 0.7m, 주기 12s, 무풍, 정방향 스웰, 오프쇼어', () => {
    const forecast: ForecastForRating = makeForecast({
      waveHeight: 0.7,
      wavePeriod: 12,
      waveDirection: 90,
...
describe('C. 경계값/엣지 케이스', () => {
  test('C1. 풍향 경계 — 179° vs 181° (S 기준 ±1°)', () => {
    /** 풍향만 바꿔가며 비교하기 위한 기본 예보 데이터 */
    const base = makeForecast({
      waveHeight: 1.5, wavePeriod: 10, waveDirection: 180,
      swellDirection: 180, windSpeed: 8, windGusts: 12, windDirection: 0,
    });
  ...
  test('C2. 360° 경계 — 풍향 0° vs 360°', () => {
    /** 풍향 0°와 360°가 동일한 결과를 내는지 확인 */
    const base: ForecastForRating = makeForecast({
  ...
  test('C3. 파고 정확히 1.2m — BEGINNER 하드블록 경계', () => {
    ...
    // 1.21m → grace zone 적용 (beach_break + 1.2~1.4m + 좋은 조건)
    // v1.4.2: BLOCKED가 아닌 WARNING으로 완화됨
    const r2 = calculateSurfRating(BEACH_BEGINNER,
      { ...forecast, waveHeight: 1.21 }, Difficulty.BEGINNER);
    expect(r2.levelFit.BEGINNER).toBe('WARNING');
  });
  ...
  test('C8. 스웰 방향 정확히 bestSwell과 일치 — swellFit 만점', () => {
    ...
    // v1.4: swellFit = dirScore(10)*0.6 + heightScore(5,null→중간)*0.25 + periodScore(5,null→중간)*0.15 = 8
    // swellHeight/swellPeriod가 null이면 중간값(5)이 적용되어 만점이 나오지 않음
    expect(result.detail.swellFit).toBe(8);
  });
  ...
  test('C9. 스웰 방향 bestSwell과 정반대 (180° 차이) — swellFit 0점', () => {
    ...
    // v1.4: swellFit = dirScore(0)*0.6 + heightScore(5,null→중간)*0.25 + periodScore(5,null→중간)*0.15 = 2
    // 방향이 정반대여도 heightScore/periodScore 중간값이 합산되어 완전 0점은 아님
    expect(result.detail.swellFit).toBe(2);
  });
  ...
  test('E1. 0~10 구간별 도달 가능 시나리오 존재 확인', () => {
    const scenarios: { name: string; spot: SpotForRating; forecast: ForecastForRating; level: Difficulty }[] = [
      ...
      { name: '0-1점', spot: BEACH_BEGINNER, level: Difficulty.BEGINNER,
        forecast: makeForecast({ waveHeight: 0.05, wavePeriod: 2, waveDirection: 270, swellDirection: 270, windSpeed: 20, windGusts: 28, windDirection: 90 })},
      // 2~3점: 작은 파도 + 보통 바람
      { name: '2-3점', spot: BEACH_BEGINNER, level: Difficulty.BEGINNER,
        forecast: makeForecast({ waveHeight: 0.3, wavePeriod: 5, waveDirection: 120, swellDirection: 120, windSpeed: 12, windGusts: 15, windDirection: 180 })},
      // 4~5점: 적당한 파도 + 방향 약간 빗나감
      { name: '4-5점', spot: BEACH_INTERMEDIATE, level: Difficulty.INTERMEDIATE,
        forecast: makeForecast({ waveHeight: 1.0, wavePeriod: 8, waveDirection: 150, swellDirection: 150, windSpeed: 10, windGusts: 14, windDirection: 90 })},
      // 6~7점: 좋은 파도 + 약간의 바람
      { name: '6-7점', spot: BEACH_BEGINNER, level: Difficulty.BEGINNER,
        forecast: makeForecast({ waveHeight: 0.8, wavePeriod: 10, waveDirection: 85, swellDirection: 85, windSpeed: 6, windGusts: 9, windDirection: 270 })},
      // 8~9점: 거의 완벽
      { name: '8-9점', spot: REEF_ADVANCED, level: Difficulty.ADVANCED,
        forecast: makeForecast({ waveHeight: 2.0, wavePeriod: 13, waveDirection: 220, swellDirection: 222, windSpeed: 3, windGusts: 5, windDirection: 30 })},
      // 9~10점: 에픽
      { name: '9-10점', spot: REEF_ADVANCED, level: Difficulty.ADVANCED,
        forecast: makeForecast({ waveHeight: 2.0, wavePeriod: 15, waveDirection: 225, swellDirection: 225, windSpeed: 1, windGusts: 2, windDirection: 30 })},
    ];
