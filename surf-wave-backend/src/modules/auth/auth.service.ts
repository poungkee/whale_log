...
export class AuthService {
   ...
   */
  /**
   * 소셜 username 자동 부여 헬퍼
   *
   * - 카카오 + 닉네임: findAvailableUsername (sanitize 통과면 그대로, 안 통과하면 시퀀스 fallback)
   * - 그 외(구글 또는 카카오 닉네임 없음): 시퀀스 fallback
   *
   * @returns username + usedFallback (시퀀스로 부여됐는지)
   */
  private async assignSocialUsername(
    provider: 'GOOGLE' | 'KAKAO',
    kakaoNickname?: string,
  ): Promise<{ username: string; usedFallback: boolean }> {
    if (provider === 'KAKAO' && kakaoNickname) {
      return this.usersService.findAvailableUsername(kakaoNickname);
    }
    const username = await this.usersService.nextGoogleTempUsername();
  ...
  async socialLogin(
    ...
    let isNewUser = false;
    /**
     * 시퀀스 fallback으로 username이 부여됐는지 — 프론트에서 "아이디 설정 팝업" 노출 판단용.
     * - 카카오 닉네임 정상 부여: false (사용자에게 알림 불필요)
     * - 카카오 닉네임 정규식 위반/구글 가입자: true ("서퍼N" 부여 → 사용자에게 변경 안내)
     */
    ...
    if (!user) {
      ...
      if (existingEmail) {
        /** 기존 이메일 가입자 → 소셜 연동만 추가 (username은 그대로 유지) */
        ...
        isNewUser = true;
        const r = await this.assignSocialUsername(provider, kakaoNickname);
        usernameAutoGenerated = r.usedFallback;
    ...
    if (user && !user.username) {
      const r = await this.assignSocialUsername(provider, kakaoNickname);
      usernameAutoGenerated = r.usedFallback;
      await this.usersService.update(user.id, { username: r.username });
