/**
 * @file guides-data.ts
 * @description Round 2 가이드 시드 데이터 — frontend Guide.tsx 5개 배열 자동 추출
 *
 * 카테고리 매핑 (frontend 카테고리 → backend GuideCategory enum):
 *   RULES_ITEMS  → ETIQUETTE  (바다 규칙/에티켓)
 *   SAFETY_ITEMS → SAFETY     (안전 상식)
 *   TERMS_ITEMS  → BEGINNER   (용어 사전 — 입문자용)
 *   BASICS_ITEMS → TECHNIQUE  (기본 자세 — 기술)
 *   GEAR_ITEMS   → EQUIPMENT  (장비 가이드)
 *
 * 시드는 GuidesModule OnModuleInit에서 INSERT IF NOT EXISTS (title 중복 시 스킵).
 */
...
/**
 * GuideItem → DB INSERT용 시드 객체 변환
 * - estimatedReadMinutes: content 길이 / 200자 기준 (최소 1분)
 * - sortOrder: 카테고리 base + 배열 인덱스
 */
function toSeedItems(
  ...
  return items.map((item, idx) => ({
    ...
    estimatedReadMinutes: Math.max(1, Math.ceil(item.content.length / 200)),
    isPublished: true,
  }));
}
...
export const GUIDE_SEED = [
  ...toSeedItems(RULES_ITEMS,  GuideCategory.ETIQUETTE, 0),
  ...toSeedItems(SAFETY_ITEMS, GuideCategory.SAFETY,    100),
  ...toSeedItems(TERMS_ITEMS,  GuideCategory.BEGINNER,  200),
  ...toSeedItems(BASICS_ITEMS, GuideCategory.TECHNIQUE, 300),
  ...toSeedItems(GEAR_ITEMS,   GuideCategory.EQUIPMENT, 400),
];
