...
**방법**: PowerShell 스크립트(`verify-time.ps1`)로 API 응답의 시간 관련 필드를 현재 UTC 시각과 비교하여 자동 검증.
**검증 시각**: 2026-02-08 01:07:22 KST (UTC 2026-02-07 16:07:22)

### Test 6-1: API 응답 시각 실시간성

API의 `fetchedAt` 필드가 요청 시점과 일치하는지 확인.

| 항목 | 값 |
|------|-----|
| API fetchedAt (UTC) | 2026-02-07 16:07:22 |
| 현재 UTC와의 차이 | -0.1초 |
| 결과 | ✅ PASS (10초 이내 — 실시간 응답) |

**의미**: `getDashboardData()`가 호출 시점에 `new Date()`로 fetchedAt을 생성하므로, 캐싱 없이 매 요청마다 실시간 데이터를 반환함.

### Test 6-2: forecastTime 과거 시점 보장

각 스팟의 `forecast.forecastTime`이 현재 시각 이전(과거)인지 확인. 서비스에서 `LessThanOrEqual(now)` 조건으로 쿼리하므로 미래 데이터가 반환되면 안 됨.
...
| 예보 시각 경과 (최소) | 7.4분 |
| 예보 시각 경과 (최대) | 7.4분 |
| 결과 | ✅ PASS (모든 forecastTime이 현재 이전) |

**의미**: TypeORM의 `LessThanOrEqual(now)` + `ORDER BY forecastTime DESC` 쿼리가 정확히 "현재 시각 직전의 가장 최신 예보"를 반환함. 미래 시점 데이터 유출 없음.

### Test 6-3: 데이터 신선도 (크론 30분 주기)

각 스팟의 `forecast.fetchedAt`이 60분 이내인지 확인. 크론이 30분마다 실행되므로 정상이라면 최대 30~35분 이내여야 함.
...
| 오래됨 (60분 초과) | 0개 |
| 가장 오래된 fetch | 7.3분 전 |
| 결과 | ✅ PASS (모든 데이터 60분 이내 수집) |

**의미**: `@Cron(CronExpression.EVERY_30_MINUTES)` 스케줄러가 정상 동작 중. 가장 오래된 데이터도 7.3분 전에 수집되어 크론 직후 상태임.

### Test 6-4: 스팟 간 forecastTime 일관성

모든 스팟이 동일한 시간대의 예보 데이터를 반환하는지 확인. 스팟마다 다른 시간대 데이터를 보여주면 대시보드 비교가 무의미해짐.

| 항목 | 값 |
|------|-----|
| 고유 forecastTime 수 | 1개 |
| 가장 이른 시각 | 2026-02-07 16:00 UTC |
| 가장 늦은 시각 | 2026-02-07 16:00 UTC |
| 시간 편차 | 0분 |
| 결과 | ✅ PASS (모든 스팟이 동일 시간대) |

**의미**: Open-Meteo API가 시간별(hourly) 데이터를 제공하고, 모든 스팟이 동일 크론 사이클에서 수집되므로 같은 forecastTime(16:00 UTC)을 참조함.

### Test 6-5: 레벨별 API 응답 속도

각 레벨의 API 응답 시간이 2초 이내인지 확인.

| 레벨 | 스팟 수 | 응답 시간 | 결과 |
|------|--------|----------|------|
| BEGINNER | 8 | 34ms | ✅ PASS |
| INTERMEDIATE | 15 | 35ms | ✅ PASS |
| ADVANCED | 18 | 39ms | ✅ PASS |
| EXPERT | 20 | 35ms | ✅ PASS |
| 미지정 | 20 | 38ms | ✅ PASS |

**의미**: 스팟 수가 8→20으로 증가해도 응답 시간이 34~39ms로 일정. `Promise.all`로 병렬 쿼리하는 구조가 효과적. 스팟이 수백 개로 증가하면 N+1 쿼리 최적화가 필요할 수 있음.
...
| 커스텀 PowerShell 스크립트 (`verify-time.ps1`) | 시간측 검증 (실시간성/신선도/일관성/성능) |
| `System.Diagnostics.Stopwatch` (.NET) | API 응답 시간 밀리초 단위 측정 |
