...
function AppInner() {
  ...
  const handleAuthSuccess = (authData: AuthResponse) => {
    ...
    /**
     * username이 시퀀스 fallback("서퍼N")으로 부여된 경우 → 아이디 설정 안내 팝업 노출.
     * - 구글 가입자: 항상 fallback (닉네임 안 받아옴)
     * - 카카오 가입자: 닉네임이 이모지/특수문자뿐이라 sanitize 후 빈 문자열인 경우만
     * - 카카오 가입자(닉네임 정상): 자동 부여됐으므로 팝업 X
     * (실제 팝업은 메인 진입 후 표시 — 온보딩 도중에는 가리지 않게)
     */
    if (authData.usernameAutoGenerated) {
